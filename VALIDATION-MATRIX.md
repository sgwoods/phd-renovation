...
| ADT batch archives (`ij1`-`ij4` era before SBCL replication) | `data/csp-adt-batch/`, snapshot batch trees | Integrity-checkable now | `tests/validate-adt-batch.sh` | Chosen as the next deliberate post-M1 integration target because it is closest to the current ADT solver line |
