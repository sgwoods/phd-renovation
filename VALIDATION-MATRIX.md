...
| ADT batch archives (`ij1`-`ij4` era before SBCL replication) | `data/csp-adt-batch/`, snapshot batch trees | Integrity-checkable now; executable investigation started | `tests/validate-adt-batch.sh`, `tests/investigate-adt-batch-bridge.sh`, `tests/investigate-adt-batch-random-lineage.sh` | Chosen as the next deliberate post-M1 integration target; first `csp`-based `ij2` bridge case now runs but diverges from preserved non-timing metrics, with random-state lineage the leading lane and the main bridge probe now defaulting to the closest surviving `qcsp3` seed bundle |
