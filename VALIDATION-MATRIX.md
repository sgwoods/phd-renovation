...
| ADT batch archives (`ij1`-`ij4` era before SBCL replication) | `data/csp-adt-batch/`, snapshot batch trees | Integrity-checkable now; executable investigation started | `tests/validate-adt-batch.sh`, `tests/investigate-adt-batch-bridge.sh`, `tests/investigate-adt-batch-random-lineage.sh`, `tests/investigate-adt-batch-input-shape.sh` | Chosen as the next deliberate post-M1 integration target; first `csp`-based `ij2` bridge case now runs, defaults to the closest surviving `qcsp3` seed bundle, and appears to diverge already in preprocessing (`Dsize`/`NCC`) rather than only later in search |
