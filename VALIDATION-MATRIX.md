...
| ADT batch archives (`ij1`-`ij4` era before SBCL replication) | `data/csp-adt-batch/`, snapshot batch trees | Integrity-checkable now; executable investigation started | `tests/validate-adt-batch.sh`, `tests/investigate-adt-batch-bridge.sh`, `tests/investigate-adt-batch-random-lineage.sh`, `tests/investigate-adt-batch-input-shape.sh`, `tests/investigate-adt-batch-source-drift.sh` | Chosen as the next deliberate post-M1 integration target; the current `ij2` investigation shows the mismatch starts in preprocessing, and `qcsp3/dist1` is presently closer to the preserved pre-search surface than `csp/dist1` |
