...
| `data/csp-adt-batch/` | Preserved ADT batch results | Yes | No, but bridge investigation started | Yes | Report-linked | Integrity-checkable preserved family; the first narrow `csp`-based `ij2` bridge now runs, but it still diverges from preserved non-timing metrics. |
