...
| `data/csp-adt-batch/` | Preserved ADT batch results | Yes | No, but bridge investigation started | Yes | Report-linked | Integrity-checkable preserved family; the first narrow `ij2` bridges now run under both maintained `csp` and maintained `qcsp3`. The mismatch still starts in preprocessing, `qcsp3` is materially closer than `csp`, a normalized `old-dist1-pre-quilici` probe is worse rather than better, and the latest structure probe shows `qcsp3` currently builds a smaller noise layer with the biggest node-domain squeeze on `t2-c`. |
