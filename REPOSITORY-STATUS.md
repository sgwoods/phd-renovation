...
| `data/csp-adt-batch/` | Preserved ADT batch results | Yes | No, but bridge investigation started | Yes | Report-linked | Integrity-checkable preserved family; the first narrow `csp`-based `ij2` bridge now runs, still diverges from preserved non-timing metrics, and now defaults to the closer surviving `qcsp3` `Rnddefault` bundle while keeping other seed sources available for comparison. |
