...
| `data/csp-adt-batch/` | Preserved ADT batch results | Yes | No, but bridge investigation started | Yes | Report-linked | Integrity-checkable preserved family; the first narrow `csp`-based `ij2` bridge now runs, still diverges from preserved non-timing metrics, and now has a dedicated random-lineage probe showing the surviving `qcsp3` `Rnddefault` bundle is materially closer than the may29 or alex copies. |
