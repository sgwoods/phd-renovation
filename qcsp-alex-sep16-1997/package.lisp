;;;; qcsp-alex-sep16-1997/package.lisp -- Package definition for Sep 1997 CSP snapshot

(defpackage #:phd-qcsp-alex
  (:use #:common-lisp)
  (:nicknames #:qcsp-alex)
...
;; AO (arc-oriented) debug — used by test4 via shared qcsp3 AO files
(defvar *ao-debug* nil)
...
;; Alex-specific counters (ADT containment, data dependency, node-type)
(defvar *component-hash* nil)
(defvar *constraint-contained-in-condition-cks* 0)
(defvar *constraint-contained-in-false-cks* 0)
(defvar *constraint-contained-in-loop-cks* 0)
(defvar *constraint-contained-in-true-cks* 0)
(defvar *constraint-control-data-dependency-cks* 0)
(defvar *constraint-guaranteed-data-dependency-cks* 0)
(defvar *constraint-possible-data-dependency-cks* 0)
(defvar *constraint-same-data-dependency-cks* 0)
(defvar *constraint-same-variable-cks* 0)
(defvar *constraint-sameline-cks* 0)
(defvar *constraint-iszero-cks* 0)
(defvar *constraint-failure* 0)
(defvar *constraint-success* 0)
(defvar *dfa-rearr-calls* 0)
(defvar *dfa-rearr-cost* 0)
(defvar *dfa-rearrangement* nil)
(defvar *node-type-consis* nil)
(defvar *node-type-consistency-checks* 0)
(defvar *node-type-consistency-ck-fail* 0)
(defvar *node-type-consistency-ck-ok* 0)
