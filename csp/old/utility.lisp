...
(defun set-globals ( raw-variables dom search-mode 
			node-consis node-force-all arc-consis
	  		fc dr adv sortc os cpu ckpt
...
(defun show-options ()
  (if (not (or (eq *output-stream* nil)
	       *single-line-override* ))
      (let ()
	(format *output-stream*
	...
	(format *output-stream*
		"~&~& Sort const option    : ~A ~&" *sort-const*)
	(format *output-stream*
...
(defun show-solution (&optional (solution-set *solution-set*) 
				(exit-location 'repeat))
  (let (
	(override-output *single-line-override*)
	)
    (if override-output
	(progn
...
"
(let (
      (situation-file  (concatenate 'string "Situation/Sit" 
		    "-" (nstring-downcase (string *sit-object-id*)) 
		    "-" (nstring-downcase (string *random-dist-name*)) 
		    "-" (nstring-downcase (string name))
		    "-" (num-to-string size) ))
      )
...
"
(let (
      (random-file  (concatenate 'string "Random/Rnd" (string-downcase (string name))))
      )
  (setq *random-stream*
	(open random-file 
	      :direction :output 
...
"
(setq *check* 15)
  (setq random-file (concatenate 'string "Random/Rnd" 
...
(let (
  ...
  (loop while (not unique) do
	...
	(if (not (probe-file (concatenate 'string "Random/Rnd" 
				 (nstring-downcase (string newstr)))))
	    (setq unique t)))
  newstr))
...
(let* (
       ...
       (red-file       (concatenate 'string "Situation/.Sit" 
			    "-" (nstring-downcase (string *sit-object-id*)) 
			    "-" (nstring-downcase (string *random-dist-name*)) 
			    "-" (nstring-downcase (string name)) 
			    "-" (num-to-string size) "." "r"))
       (green-file     (concatenate 'string "Situation/.Sit" 
			    "-" (nstring-downcase (string *sit-object-id*)) 
			    "-" (nstring-downcase (string *random-dist-name*)) 
			    "-" (nstring-downcase (string name)) 
			    "-" (num-to-string size) "." "g"))
       (yellow-file    (concatenate 'string "Situation/.Sit" 
			    "-" (nstring-downcase (string *sit-object-id*)) 
			    "-" (nstring-downcase (string *random-dist-name*)) 
			    "-" (nstring-downcase (string name)) 
			    "-" (num-to-string size) "." "y"))
       (purple-file    (concatenate 'string "Situation/.Sit" 
			    "-" (nstring-downcase (string *sit-object-id*)) 
			    "-" (nstring-downcase (string *random-dist-name*)) 
			    "-" (nstring-downcase (string name)) 
