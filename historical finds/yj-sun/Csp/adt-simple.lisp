...
(defun adt (&key
	    ...
	    (sit-noise                     0)
	    (template-id 
	     "average-array-template-9-17");; special implies o/ride
...
(let (
  ...
  (cond   ;; Constraint type
   ...
   ;; April 13, 1997, Yongjun Zhang 

   ;;; -----------------------------------------------------------------------
   ;; real program, contained-in-condition (component cotained in condition?) 
   ((eq cons-type 'contained-in-condition)

    (setq *constraint-cks* (+ 1 *constraint-cks*))
    (if  *debug-consis* (comment1 " " *constraint-cks*))
    (setq *constraint-contained-in-condition-cks* 
			(+ 1 *constraint-contained-in-condition-cks*))

    (check-contained-in-condition constraint ts1 s1 ts2 s2))


   ;;; -----------------------------------------------------------------------
   ;; real program, contained-in-true (contained in true part of a condition?) 
   ((eq cons-type 'contained-in-true)

    (setq *constraint-cks* (+ 1 *constraint-cks*))
    (if  *debug-consis* (comment1 " " *constraint-cks*))
    (setq *constraint-contained-in-true-cks* 
			(+ 1 *constraint-contained-in-true-cks*))

    (check-contained-in-true constraint ts1 s1 ts2 s2))
 
   ;;; -----------------------------------------------------------------------
   ;; real program, contained-in-false (contained in false part of a condn ?) 
   ((eq cons-type 'contained-in-false)

    (setq *constraint-cks* (+ 1 *constraint-cks*))
    (if  *debug-consis* (comment1 " " *constraint-cks*))
    (setq *constraint-contained-in-false-cks* 
			(+ 1 *constraint-contained-in-false-cks*))

    (check-contained-in-false constraint ts1 s1 ts2 s2))

   ;;; -----------------------------------------------------------------------
   ;; real program, contained-in-loop (component cotained in body?) 
   ((eq cons-type 'contained-in-loop)

    (setq *constraint-cks* (+ 1 *constraint-cks*))
    (if  *debug-consis* (comment1 " " *constraint-cks*))
    (setq *constraint-contained-in-loop-cks* 
			(+ 1 *constraint-contained-in-loop-cks*))

    (check-contained-in-loop constraint ts1 s1 ts2 s2))
...
(defun check-same-data-dependency (constraint t1 s1 t2 s2 )
  (let* (
	 ...
	 (s1-var  (get-sit-variable s1 t1-posi))  ; Added by Yongjun Zhang  
	 (s2-var  (get-sit-variable s2 t2-posi))  ; on 4/25/97 
	 (same-var (equal s1-var s2-var))         ; ??  
	 (same-dependency (and same-var (compare-dependency s1-dependency s2-dependency)))   
	)           
...
;; return the test component list of a component (Test or Loop)
(defun get-test-comp-list ( comp-name )
  (nth 2 (nth 0 (gethash comp-name *component-hash*))))

;; return the component list of the true branch of a Test or the Loop body
(defun get-test-true-list (comp-name )
  (nth 3 (nth 0 (gethash comp-name *component-hash*))))

;; return the component list of the true branch of a Test
(defun get-test-false-list (comp-name )
  (nth 4 (nth 0 (gethash comp-name *component-hash*))))
   
;; ***************************************************************************
;; (contained-in-condition (comp-a comp-b))
;; comp-b should be a Test or a Loop, each of which has a slot for the test
;; statement (generated from the test expression). This constraint means 
...
;; ***************************************************************************
;; (contained-in-true (comp-a comp-b))
;; comp-b should be a Test, which has a slot containing a list of names of all 
;; components contained in the true branch of a Test. This constraint means 
...
;; ***************************************************************************
;; (contained-in-false (comp-a comp-b))
;; comp-b should be a Test, which has a slot containing a list of names of all 
;; components contained in the false branch of a Test. This constraint means 
;; comp-a is a component in the false branch.
...
;; ***************************************************************************
;; (contained-in-loop (comp-a comp-b))
;; comp-b should be a Loop, which has a slot containing a list of names of all 
;; components contained in the body of the Loop. This constraint means 
;; comp-a is a component in the body.
...
;;			)
;;		    (if (and (or-equal so-declArrType ts-arr-type-list)
...
;;		      )
;;		  (if (or-equal so-declType ts-type-rng)
;;			t
;;		      nil)  )))))
;;
      ;;  Zero
...
(comment2 "objlist, ident = " objlist ident)
