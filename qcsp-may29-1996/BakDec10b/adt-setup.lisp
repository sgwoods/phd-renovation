...
(defun situation-setup (sit-id sit-noise
  ...
  (let (
	...
	(progn   ;; reload condition
	  (comment "Reload situation from existing file.")
	  (if (not (numberp sit-noise))
		...
		(progn
		  (setq *current-situation*
			(read (open situation-file :direction :input)))
		  (return-from situation-setup t)
		  )
	      (progn
		(comment1 "Sit noise < 0, cant relaod" sit-noise)
		(return-from situation-setup nil)
		)
	      )))
      ;;
	...
	(comment "Generate new situation and save it.")
