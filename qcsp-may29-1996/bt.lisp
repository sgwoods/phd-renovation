...
;;					:parent-state     parent-state) )) 
		    ...
		    (block instantiate-next-variable
			   ...
			   (when forward-checking
			     ...
			     (if *debug*
				 (comment1 "FC varleft out" variables-left)) )

			   ;; DYNAMIC REARRANGEMENT sort
			   (when dynamic-rearrangement
			     (if *debug*
				 (comment1 "DR varleft input" variables-left))
			     (setq variables-left (dynamic-rearrangement
						   variables-left))
 			     (if *debug*
				 (comment1 "DR varleft out" variables-left)) )

			   ;; ------------------------------
			   ;; Select the next symbol/variable to instantiate
			   (setq symbol (get-first-symbol variables-left))
			   (setq domain (cdar variables-left))

			   (if *debug* 
			       (comment2 '"Next Symbol, domain" symbol domain))
...
"
  ...
  (if *debug-consis* 
      (comment3 "Satisfy-p <symbol, value, partial>" symbol value partial-solution))
