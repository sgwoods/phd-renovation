...
(defun set-globals ( raw-variables 
  ...
  (if (member-string *search-mode* '("bt" "bm" "bj" "qu"))
  ...
  (let (
	...
	(progn
	  ...
	  (if (not (eq *output-stream* nil))
		...
		(format *output-stream* 
	  "~A~9T~A/~12T~A/~16T~A/~20T~A/~24T~A~29T~A~41T~A~49T~A~57T~A~65T~A / ~A~76T~A~84T~A~92T~A~101T~A~109T~A~114T~A/~A~124T~A~135T~A~144T~A/~A~156T~A~165T~A~&" 
