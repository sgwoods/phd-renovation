...
(defun load-applyr (&optional (sys *unix*) )
  (if sys
    ...
    (progn
      (load  "Macintosh HD:Languages:Allegro Common Lisp:qcsp:applyr.lisp")    
      (load "Macintosh HD:Languages:Allegro Common Lisp:qcsp:comment.lisp"))
      ))

(defvar *debug-app* t)  ;; 
(setq   *debug-app* t)  ;; TURN trace of applyR on 

(defun applyR ( xi yj relation curMode maxDepth  curDepth &key (markflg t))
" 
...
;; (if *debug-app*
;;    (comment3 "ApplyR call : xi, yj, depth" 
;;	  (if xi (DomVal-Name xi) nil)  (if yj (DomVal-Name yj) nil)   curDepth))
	...
	(if (fail3-p Rlocal)    ;; ie Rlocal = fail
	    (progn
		  ...
		  (let* (
		    ...
		    (if xlp (markit xi xlp 'lp))
		    (if xrp (markit xi xrp 'rp))
		    (if xlc (markit xi xlc 'lc))
		    (if xrc (markit xi xrc 'rc))
		    ))
	      ;; return
	  ...
	  (cond
	   ...
	   ( (eq curMode 'initial)
	     (setq return-val
		   (let* (  (dn (applyDown xi yj relation maxDepth curDepth markflg))  )
		     (if (fail3-p dn)
			 'fail
		       (let ( (up (applyUp xi yj relation maxDepth curDepth markflg)) )
			 (and3 (list dn up)) )))) )
     
	   ( (eq curMode 'down)
	     (setq return-val (applyDown xi yj relation maxDepth curDepth markflg)) )

	   ( (eq curMode 'up)
	     (setq return-val (applyUp   xi yj relation maxDepth curDepth markflg)) )
...
;;			 (DomVal-Name xi) (DomVal-Name yj) curDepth))
  ...
  (let* (
	...
	(let* (
	       ...
	       (RLL (applyR xlp ylp relation 'up maxDepth newDepth))
	       (RLR (applyR xlp yrp relation 'up maxDepth newDepth))
	       (RRL (applyR xrp ylp relation 'up maxDepth newDepth))
	       (RRR (applyR xrp yrp relation 'up maxDepth newDepth))
...
;;			  (DomVal-Name xi) (DomVal-Name yj) curDepth))
  ...
  (let* (
	...
	(let* (
	       ...
	       (xrc    (DomVal-RightChild xi))
;;	       (xrc-type (DomVal-RightChildType xi))
	       (ylc    (DomVal-LeftChild  yj))
	       (ylc-type (DomVal-LeftChildType yj))
	       (yrc    (DomVal-RightChild yj))
;;	       (yrc-type (DomVal-RightChildType yj))

	       (RLL (applyR xlc ylc relation 'down maxDepth newDepth))
	       (RLR (applyR xlc yrc relation 'down maxDepth newDepth))
	       (RRL (applyR xrc ylc relation 'down maxDepth newDepth))
	       (RRR (applyR xrc yrc relation 'down maxDepth newDepth))
    	       )
	  ...
	  (cond
	   ...
	   ( (and (eq xlc-type 'and) (eq ylc-type 'or))
	     (let* (
		   ...
		   (progn
		     (if markflg (markit   xi xlc 'lc))
		     (if markflg (markit   xi xrc 'rc))
	   ...
	   ( t
		 ;; return ERROR
	     (progn
	       (comment2 "Error in graph from condition: xtype ytype" xlc-type ylc-type)
	       (setq return-val 'error-in-graph-from-cond)))

	   ) ; cond

;;	  (if *debug-app* (comment3 "applyDown xi yj return-val" 
;;				    (DomVal-Name xi) (DomVal-Name yj) return-val))
    ...
    (let* (
	 ...
	 (if (eq thisval 'poss)
	   ...
	   (if (not (eq thisval 'fail))
	       (progn
		 (comment3 "Error in or3: thisform thisval nil" thisform thisval nil)
		 (setq sofar 'fail)
		 ))))
       ) ; loop
    ...
    (let* (
	 ...
	 (if (eq thisval 'poss)
	   ...
	   (if (not (eq thisval 'true))
	       (progn
		 (comment3 "Error in and3: thisform thisval nil" thisform thisval nil)
		 (setq sofar 'fail)
		 ))))
       ) ; loop
