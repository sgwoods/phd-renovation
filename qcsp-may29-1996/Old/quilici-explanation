...
" Some random function with random types of parameters"
  (let* (
	 ...
	 (end-sloc    (+ sloc (random max-begin-space)))
	)
    (list
...
(defun generate-ran-Stmt ( sid sloc index basePair baseType)
  ...
  (let ( 
    ...
    (if (eq sel 0)
	(generate-ran-Zero sid (1+ sloc) index)
      (if (eq sel 1)
	  (generate-ran-Assign sid (1+ sloc) index basePair)
	(if (eq sel 2)
	    (generate-ran-Increment sid (1+ sloc) index)
	  (generate-ran-Print-arr sid (1+ sloc) index))))))

(defun generate-ran-For (sid sloc)
  (let* (
	 (max-begin-space   15)              ;; how much room in the for ?

	 (other-sid  (gentemp "other-sid"))
	 (other-sloc (+ (random max-begin-space) sloc))

	 (block-name (create-block-name))
	 (begin-sid  (gentemp "begin-sid"))
	 (begin-sloc (1+ sloc))              ;; begin right after for

	 (end-sid  (gentemp "end-sid"))
	 (end-sloc (1+ other-sloc))
...
(defun generate-ran-While (sid sloc)
  (let* (
	 (max-begin-space   15)              ;; how much room in the while ?

	 (begin-sid  (gentemp "begin-sid"))
	 (begin-sloc (1+ sloc))              ;; begin right after while
	 (other-sloc (+ (random max-begin-space) sloc))
	 (block-name (create-block-name))
  	 (end-sid  (gentemp "end-sid"))
	 (end-sloc (1+ other-sloc))
...
(defun adt-set-global-values (sit-id sit-noise random-ident temp-id 
...
(let (
  ...
  (if (not (numberp sit-noise))
	...
	(let (
	  ...
	  ;; sort situation list by line number, replace old line nos with new
	  (setq *original-situation* *current-situation*) 
	  (setq *current-situation*  (restructure-line-numbers
				      *current-situation*))
	  )
...
"
(let (
      (situation-file  (concatenate 'string "ADT-Situation/Sit" 
...
"
(let (
      (random-file  (concatenate 'string "ADT-Random/Rnd" (string-downcase (string name))))
      )
  (setq *random-stream*
	(open random-file 
	      :direction :output 
