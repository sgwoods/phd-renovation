 ...
 *  CPU Seconds version
 *
 * example ...   grab-ci-time < file.input > file.output
 *
 * Steve Woods, Feb 1995
 */
...
main()
{
	...
	for (i=0; i<ITERS; i++)	{
	    ...
	    TCCf = Total;   /* TOTAL CPU SECONDS , includes NC pre-processing etc etc */
	    sum = sum + TCCf;
...
/* Local Variables: */
/* compile-command: "gcc grab-ci-time.c" */
/* End: */
