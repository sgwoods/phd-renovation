...
| AO baseline breadth | `qcsp3/` has asserted AO coverage for the supported deterministic baseline | AO extras / `run-test4*.lisp` paths across snapshots | AO is part of the integrated story, but the full historical AO surface is broader than the current asserted case. | `tests/ao-run.lisp` and `tests/validate-ao.sh` cover the supported baseline; broader AO coverage remains explicitly open. | Reduce after M1 |
...
| ADT batch archives before the SBCL replication line | not yet validated through a modern path | `data/csp-adt-batch/`, historical batch trees | These likely contain useful historical coverage, but they are still archive-heavy and workflow-light. | Preserved and cataloged, but still provenance-only in the current matrix. | Reduce after M1 |
...
| QCSP-nov96 terrain domain | missing | `QCSP-nov96.dir`, `data/lost-datasets.md` | This is the clearest missing historical domain, but source/data loss prevents current integration. | Documented as blocked by missing artifacts. | Blocker outside M1 |
| Validation-log noise | `qcsp3/` baseline is now much quieter; remaining routine noise is mostly redefinition chatter in AO loads and a small alex `adt-setup` note | mostly AO load paths and alex setup | A noisy merge gate makes regressions harder to notice, even when behavior is correct. | The large alex/core STYLE-WARNING tail is cleared; what remains is mostly cosmetic but still worth trimming. | Reduce after M1 |
